    ...
    <div style={{ background: theme.bg, minHeight: '100vh', color: theme.text, fontFamily: theme.fontBody }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 24px' }}>
        ...
        <div style={{ textAlign: 'center', padding: '56px 20px 40px' }}>
          ...
          <div style={{ fontSize: 48, fontWeight: 200, color: '#fff', lineHeight: 1, marginBottom: 8, fontFamily: theme.fontMono }}>
            {Math.floor(score.score_value)}<span style={{ fontSize: 28, color: theme.textTertiary }}>.{(score.score_value % 1).toFixed(2).slice(2)}</span>
          </div>
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, marginBottom: 24 }}>
            {score.delta !== null && (
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 12px', borderRadius: 4, background: `${bandColor}15`, border: `1px solid ${bandColor}30` }}>
                <span style={{ fontFamily: theme.fontMono, fontSize: 13, color: bandColor }}>{score.delta > 0 ? '▲' : '▼'} {Math.abs(score.delta).toFixed(2)}</span>
        ...
        <div className="grid-movers" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 24 }}>
          {[...topRisers, ...topFallers].map(m => {
            ...
            return (
              <div key={m.domain} style={{ background: theme.surface, border: `1px solid ${theme.surfaceBorder}`, borderRadius: 10, padding: 16 }}>
        ...
        <div className="stat-bar" style={{
          ...
          background: theme.surface, border: `1px solid ${theme.surfaceBorder}`, borderRadius: 10, overflow: 'hidden',
        }}>
          {[
        ...
        <Link href={`/pulse/${pulse.slug}`} style={{ textDecoration: 'none', display: 'block', marginBottom: 24 }}>
          <div style={{ background: theme.surface, border: `1px solid ${theme.surfaceBorder}`, borderRadius: 10, padding: 24, cursor: 'pointer' }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 600, color: '#fff', marginBottom: 8, fontFamily: theme.fontHeading }}>{pulse.title}</div>
        ...
        {/* ═══ Quiz + Subscribe ═══ */}
        <div className="grid-2col" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
          <div style={{ background: `linear-gradient(135deg, #0a1a0a, ${theme.surface})`, border: `1px solid ${theme.accent}33`, borderRadius: 10, padding: 32, textAlign: 'center' }}>
