...
async function loadCountries(): Promise<CountryRow[]> {
  ...
  // Fetch latest composite_score_id per country (one query per country, parallelized)
  ...
  for (const r of idResults) {
    if (r) idByCountry.set(r.country_code, r.id);
  }
  ...
  // Build map countryCode → { meta: value }
